...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def compute_target_value(self, next_obs, rewards, terminals, actor):
        ...
        ac_act = actor(next_obs).sample()
        q_target_values = self.target_network(torch.cat((next_obs, ac_act), axis=-1)).squeeze(1)
        target_value = (rewards + self.gamma * q_target_values * (1 - terminals)).detach()
    ...
    def update_target_network_ema(self):
        ...
        # your code here
        # data averaging
        target_param = torch.cat([param.data.view(-1) for param in self.target_network.parameters()],0)
        critic_param = torch.cat([param.data.view(-1) for param in self.critic_network.parameters()],0)
        new_param = (1-self.target_update_rate) * target_param + self.target_update_rate * critic_param

        # load data
        i = 0
        for param in self.target_network.parameters():
            param.data.copy_(new_param[i:i + param.nelement()].view(param.size())).view(param.size())
            i += param.nelement()
