...
class DQNCritic(BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        if self.double_q:
            ...
            # your code here
            ac_cur = torch.argmax(self.q_net(next_ob_no), dim=1)
            qa_tprime_values = self.q_net_target(next_ob_no)
            q_tprime_values = torch.gather(qa_tprime_values, 1, ac_cur.unsqueeze(1)).squeeze(1)
            ...
            # your code here
            qa_tprime_values = self.q_net_target(next_ob_no)
            q_tprime_values, _ = torch.max(qa_tprime_values, 1)
        ...
        target = (reward_n + self.gamma * q_tprime_values * (1 - terminal_n)).detach()
