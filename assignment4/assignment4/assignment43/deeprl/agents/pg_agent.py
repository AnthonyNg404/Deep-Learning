...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_returns = np.sum([i*self.gamma**list(rewards).index(i) for i in rewards])
        list_of_discounted_returns = np.full(rewards.shape, discounted_returns)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounted_cumsums = [sum([j*self.gamma**(list(rewards).index(j) - i) for j in rewards[i:]]) for i in range(rewards.shape[0])]
        list_of_discounted_cumsums = np.asarray(discounted_cumsums)
